class IRiSArchive {
  constructor() {
    ...
    this.currentYear = '2018'; // 기본값은 2018년 이후
    this.currentSearch = '';
    this.lastSearch = '';
    this.lastYear = '2018';
    this.allPostGroups = [];
  ...
  applyFilters() {
    ...
    if (this.currentYear !== 'all') {
      const targetYear = parseInt(this.currentYear);
      if (targetYear === 2018) {
        // 2018년 이후 (2018년 이상)
        filtered = filtered.filter(post => post.year >= targetYear);
      } else {
        // 특정 연도만
        filtered = filtered.filter(post => post.year === targetYear);
      }
    ...
    // 검색이나 연도 필터가 바뀐 경우에만 페이지 리셋
    if (this.lastSearch !== this.currentSearch || this.lastYear !== this.currentYear) {
      this.currentPage = 1;
    }
  ...
  createPostHTML(post) {
    ...
    if (post.attachment && post.attachment.trim()) {
      ...
      if (!processedContent.includes(filename)) {
        processedContent += `<br><img src="images/${post.year}/${filename}" alt="${post.title}" ${styleClass}>`;
      }
    }
    // 본문 클래스 구분
