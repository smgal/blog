  ...
  createPostHTML(post) {
    ...
    if (post.attachment && post.attachment.trim()) {
      ...
      if (!processedContent.includes(filename)) {
        processedContent = `<img src="images/${filename}" alt="${post.title}" ${styleClass}><br>` + processedContent;
      }
    }
    
    // 작성자 정보 HTML 생성
    let authorDisplay = this.escapeHtml(post.name);
    if (post.email) {
      authorDisplay = `<a href="mailto:${post.email}" title="${post.email}">${authorDisplay}</a>`;
    }
    if (post.url) {
      authorDisplay += ` <a href="${post.url}" target="_blank" rel="noopener noreferrer" title="${post.url}">🌐</a>`;
    }

    // 댓글 제목 처리
    if (isReply && post.title && post.title.trim()) {
      processedContent = `<strong>${this.escapeHtml(post.title)}</strong><br>` + processedContent;
    }
